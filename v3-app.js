...
(function () {
  ...
  /* ── the watched board ────────────────────────────────────────────────────
     The section says you need someone watching the game you are in. A spinning
     ornament next to that sentence proves nothing, so this proves it: pick one
     of three real moves and GM Forge plays it out and tells you what it runs
     into.

     Every line and every resulting position came out of Stockfish at depth 18
     (see v3-demo.json, generated by the build step) -- Ng5 really does hang the
     knight to Qxg5 at nearly minus four, and Nxe5 really does lose a knight for
     a pawn. Nothing here is invented, which is the same rule the product holds
     itself to. */
  (function watched() {
    var board = $('#wboard'), btns = $('#wbtns'), text = $('#wtext'),
        reset = $('#wreset'), say = $('#wsay');
    if (!board || !window.fetch) return;
    var S = null;
    function paint(key) {
      var st = S && S[key]; if (!st) return;
      var cells = $$('.sq', board);
      st.cells.forEach(function (p, i) {
        var c = cells[i]; if (!c) return;
        c.innerHTML = p ? '<img src="pieces/' + p + '.svg" alt="">' : '';
        c.classList.toggle('ring', st.ring === c.dataset.sq);
        c.classList.toggle('trail', st.from === c.dataset.sq || st.to === c.dataset.sq);
      });
      if (st.say) {
        text.textContent = st.say;
        say.classList.toggle('good', !!st.good);
        say.classList.toggle('bad', !st.good);
      }
    }
    fetch('v3-demo.json').then(function (r) { return r.json(); }).then(function (d) {
      S = d; paint('start');
      btns.addEventListener('click', function (e) {
        var b = e.target.closest('button[data-mv]'); if (!b) return;
        paint(b.dataset.mv);
        btns.hidden = true; reset.hidden = false;
      });
      reset.addEventListener('click', function () {
        paint('start');
        text.textContent = 'Pick one and I will play it out. This is the part a video ' +
          'cannot do for you \u2014 it has to happen to your move, in your game.';
        say.classList.remove('good', 'bad');
        btns.hidden = false; reset.hidden = true;
      });
    }).catch(function () {});
  })();
})();
